...
contract EventTicketsV2 {
    ...
    */
     function buyTickets(uint eventId, uint numberOfTickets) public payable {
        require(events[eventId].isOpen == true,'The Event is not open');
        require(msg.value >= numberOfTickets*PRICE_TICKET, 'Is not fund for the number of tickets purchased');
        require(events[eventId].totalTickets - events[eventId].sales >= numberOfTickets,'There is not available tickets');
        events[eventId].buyers[msg.sender] += numberOfTickets;
        events[eventId].sales += numberOfTickets;
        uint refund = msg.value - numberOfTickets*PRICE_TICKET;
    ...
    */
    ...
    function getRefund(uint eventId) public payable {
        ...
        events[eventId].sales -= ticketsPurchased;
    ...
    */
    function getBuyerNumberTickets(uint eventId) public view returns(uint purchasedTickets) {
